...
class CNW_AnalysishutangREPORT(models.TransientModel):
    _name           = "cnw.awr28.analysishutang"
    _description    = "cnw.awr28.analysishutang"
    ...
    @api.multi
    def view_awr28_analysishutang(self): 
        mpath       = get_module_path('cnw_awr28')
        filenamexls = 'sls_analysishutang_'+   self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamepdf = 'sls_analysishutang_'+   self.dateto.strftime("%Y%m%d")  + '.pdf'
        filename    =""
        filepath    = mpath + '/temp/'
        ...
        for comp in self.company_id:
            ...
            msg_sql= "exec dbo.IGU_ACT_analysishutang   '"+ self.datefrom.strftime("%Y%m%d")   + "','"+ self.dateto.strftime("%Y%m%d")   + "','" + comp.code_base  + "'"
