...
class CNW_HPPSUMMARY(models.TransientModel):
	_name           = "cnw.awr28.hppsummary"
	_description    = "cnw.hppsummary"
		...
		for comp in self.company_id:
			...
			msg_warehouse = """ declare 
								...
								select      @company Company,
											...
											c.U_speGroup ,
											c.U_COuntry,
											replace(replace (c.U_Spec, char(13),''),char(10),'') Spec, 
											replace(replace (convert(varchar,c.U_HS_Code), char(13),''),char(10),'') HSCode,
											a.locCode WhsCode, 
								...
								and c.itemcode + c.itemname + c.u_group like '%' + isnull(@item,'') + '%'  -- and b.transValue<>0
									and  isnull(c.u_group,'') like  '%' + isnull(@igroup,'') + '%' 
								group by    c.ItemCode,
											c.ItemName, 
											c.U_Group ,
											c.U_SubGroup ,
											c.U_speGroup ,
											c.U_COuntry,
											replace(replace (c.U_Spec, char(13),''),char(10),'') , 
											replace(replace (convert(varchar,c.U_HS_Code), char(13),''),char(10),'') ,
											a.locCode
			...
			if self.export_to =="hs01":
				msg_sql=  """
								...
								and b.u_group like '%' +  isnull(@igroup,'') + '%'
								group by 
										   ISNULL(CONVERT(VARCHAR,b.u_hs_code),'')  , 
											b.U_SPEGROUP ,
										upper(b.invntryUom)   
