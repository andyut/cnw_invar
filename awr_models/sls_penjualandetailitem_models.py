...
class CNW_penjualandetailitem(models.TransientModel):
    _name           = "cnw.awr28.penjualandetailitem"
    ...
    export_to       = fields.Selection([    ('xls', 'Excel-Detail'),('xlsshort', 'Excel-Short'),
                                            ...
                                            ('hs_realisasi_summary','HS Realisasi summary'),
    ...
    def view_penjualandetailitem(self): 
        ...
        igroup      = self.igroups if self.igroups else ""
        ...
        for comp in self.company_id:
            ...
            msg_sql= """
                            ...
                            SELECT  
                                    ...
                                    f.quantity ,
                                    isnull(f.U_Qty_AR ,0) QtyReceive,
            ...
            """
            if self.export_to == "hs_realisasi" or self.export_to == "hs_realisasi_summary" :
                msg_sql ="""
                            declare 
                                    @datefrom	varchar(10),
                                    @dateto		varchar(10),
                            ...
                            and  isnull(c.u_group ,'')  +    isnull(c.u_subgroup,'')  like '%' + isnull(@group,'') + '%'
                            and c.itemcode + c.itemname like '%' + isnull(@item,'') + '%'
        ...
        if self.export_to =="xlsshort":
            filename = filenamexls 
            dfsimple = df["invoice","partner_group","outlet","partnercompany","sales_in_trx","itemname","uom","quantity","QtyReceive","price","Total","margin"]
            dfsimple.loc['Total'] = dfsimple.select_dtypes(pd.np.number).sum().reindex(dfsimple.columns, fill_value='')
            dfsimple.to_excel(mpath + '/temp/'+ filenamexls)  
        ...
        if self.export_to =="xls-summary3":
            filename = filenamexls 
            pivottbl = df.pivot_table(index=["Company","itemname"],columns=["imonth"],aggfunc=np.sum,  values=["Total"],fill_value="0",margins=True)
            pivottbl.to_excel(mpath + '/temp/'+ filenamexls)  
                     
        if self.export_to =="xls-summary4":
            filename = filenamexls 
            pivottbl = df.pivot_table(index=["Company","sales_in_trx"],columns=["imonth"],aggfunc=np.sum,  values=["Total"],fill_value="0",margins=True)
            pivottbl.to_excel(mpath + '/temp/'+ filenamexls)  

        if self.export_to =="xls-summary5":
            filename = filenamexls 
            pivottbl = df.pivot_table(index=["Company","sales_in_trx","partnercompany"],columns=["imonth"],aggfunc=np.sum,  values=["Total"],fill_value="0",margins=True)
            pivottbl.to_excel(mpath + '/temp/'+ filenamexls)  
        ...
        if self.export_to =="hs_realisasi_summary":
            filename = filenamexls 
            pivottbl = df.pivot_table(index=["Company","hs_code","spegroup","subgroup"], aggfunc=np.sum,  values=["quantity"],fill_value="0",margins=True)
            pivottbl.to_excel(mpath + '/temp/'+ filenamexls)  
