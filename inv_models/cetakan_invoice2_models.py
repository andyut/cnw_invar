...
class CNWCetakanInvoice2(models.TransientModel):
    _name           = "cnw.cetakan.invoice2"
    _description    = "Cetakan Invoice2"
    company_id      = fields.Many2one('res.company', 'Company', required=True, index=True,  default=lambda self: self.env.user.company_id.id) 
    kwitansi        = fields.Char("Kwitansi",default="",required=True) 
    filexls         = fields.Binary("File Output")    
    filenamexls     = fields.Char("File Name Output")

    def get_CetakanInvoice(self):
        mpath       = get_module_path('cnw_invar') 
        filenamepdf    = 'invoice2_'+   self.kwitansi   +  self.env.user.name +  '.pdf'
        filenamepdf    = 'invoice2_'+   self.kwitansi   +  self.env.user.name +   '.pdf'
...
#MULTI COMPANY 
        ...
        msgsql =  """exec [dbo].[IGU_ACT_INVOICE_HEADER_KWITANSI]  '""" + self.kwitansi +  """' """
        msgsql2 =  """exec [dbo].[IGU_ACT_INVOICE_DETAIL_KWITANSI]  '""" + self.kwitansi +  """' """
