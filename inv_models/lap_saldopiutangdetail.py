...
# print invoice
	...
	def get_CetakanInvoice(self):
		...
		filenamepdf    = 'invoice_'+   self.docentry     +  self.env.user.name +   '.pdf'
		filenamejson    = 'invoice_'+   self.docentry     +  self.env.user.name +   '.pdf'
		filenamexlsx    = 'invoice_'+   self.docentry     +  self.env.user.name +   '.xlsx'
...
class CNWLapSaldoPiutangDetail(models.TransientModel):
	...
	filexls         = fields.Binary("File Output",default=" ")    
	filenamexls     = fields.Char("File Name Output",default="EmptyText.txt")
	nomorsurat 		= fields.Char("Nomor Surat")
	tanggalsurat	= fields.Date("Tanggal Surat",default=lambda s:fields.Date.today())
	...
	export_to       = fields.Selection([ ('list','List 	'),
				     						('xlssummary', 
											'Excel Raw Data Summary'),
											('xls', 'XLS Raw Data'),
											('json', 'JSON Raw Data'),
											('pdf', 'PDF Report'),
											('xls2', 'Microsoft Excel (xlsx)'),
											('rtf', 'Microsoft RTF (RichText)'),
											('surat', 'Surat Tagihan'), 
											('docx', 'Microsoft Word (Docx)')
											],string='Export To', default='list')
...
#MULTI COMPANY x
		...
		for comp in self.company_id:
			...
			msgsql ="""
						...
						select  
								...
								isnull(b.U_PrintKwitansi,'') printkwitansi
		...
		if self.export_to =="xls2":
			...
			input_file 		= mpath + '/temp/' +  filex + ".jrxml" 
			data_file 		= mpath + '/temp/' +  filex + ".json" 
			output_file 	= mpath + '/temp/' +  filenamexls2
			filename 		= filenamexls2 
		...
		if self.export_to =="docx":
				   
			proyeksi = self.env["cnw.invar.jasper"].search([("name","=","saldopiutangdetail")])
			input_file 		= mpath + '/temp/' +  filex + ".jrxml" 
			data_file 		= mpath + '/temp/' +  filex + ".json" 
			output_file 	= mpath + '/temp/' +  filenamedocx
		...
		if self.export_to =="rtf":
				   
			proyeksi = self.env["cnw.invar.jasper"].search([("name","=","saldopiutangdetail")])
			input_file 		= mpath + '/temp/' +  filex + ".jrxml" 
			data_file 		= mpath + '/temp/' +  filex + ".json" 
			output_file 	= mpath + '/temp/' +  filenamertf
			filename 		= filenamertf 
		...
		if self.export_to =="surat":
				   
			proyeksi = self.env["cnw.invar.jasper"].search([("name","=","surattagihan")])
			input_file 		= mpath + '/temp/' +  filex + ".jrxml" 
			data_file 		= mpath + '/temp/' +  filex + ".json" 
			output_file 	= mpath + '/temp/' +  filenamedocx
			filename 		= filenamedocx 
		...
		for comp in self.company_id:
			...
			msgsql ="""
						...
						select  a.transid , 
								...
								f.slpName + ' ' + isnull(f.u_slsEmpName,'') salesname ,
								isnull(b.notes,'') jadwal,
								isnull(a.linememo,'') notes1, 
								isnull(a.linememo,'') notes2 
