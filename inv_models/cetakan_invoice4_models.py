...
class CNWCetakanInvoice4(models.TransientModel):
    _name           = "cnw.cetakan.invoice4"
    _description    = "Cetakan invoice4"
    ...
    partner         = fields.Char("Partner",default="",required=True) 
    ...
    def get_CetakanInvoice(self):
        ...
        filenamepdf    = 'invoice_'+    self.env.user.name +  '.pdf'
        filenamepdf    = 'invoice_'+    self.env.user.name +   '.pdf'
...
#MULTI COMPANY 
        ...
        msgsql =  """exec [dbo].[IGU_ACT_INVOICE_HEADER4]  '""" + self.partner +  """','""" + self.datefrom.strftime("%Y%m%d")  +  """','""" + self.dateto.strftime("%Y%m%d") +  """' """
        msgsql2 =  """exec [dbo].[IGU_ACT_INVOICE_DETAIL4] '""" + self.partner +  """','""" + self.datefrom.strftime("%Y%m%d")  +  """','""" + self.dateto.strftime("%Y%m%d") +  """' """
