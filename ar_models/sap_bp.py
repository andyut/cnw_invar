	...
	def view_bp_xls(self): 
		...
		if database =="IGU_NEW":
			...
			msgsql = """
							...
							select                  '"""+ database + """' +  convert(Varchar,a.docentry) id ,  
													 '"""+ database + """' +  convert(Varchar,a.docentry) name ,  
													a.cardcode , 
													a.cardname , 
													isnull(a.cardfname,'') cardfname,  
													b.groupname ,  
...
#        cursor.execute( """ exec  [dbo].[IGU_ACT_BUSINESSPARTNER] '""" + partner + """' """ )
		...
		rowdata = cursor.fetchall()
		print ( type(rowdata))
		#self.env.cr.execute ("""DELETE FROM sap_bp WHERE create_uid =""" + str(self.env.user.id) + """ """ ) 
