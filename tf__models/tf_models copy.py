...
class ARTukarfakturWizard(models.TransientModel):
	...
	ar_id           = fields.Many2one("res.users",string="AR Person", default=lambda self: self.env.user.id )
	docdate         = fields.Date("Date",default=lambda s:fields.Date.today())
	statustf        = fields.Selection(string="Status",selection=[("Tukar Faktur","Tukar Faktur"),("Giro","Giro"),("Lain-lain","Lain Lain")],default="Tukar Faktur")       

	@api.multi
	def CreateJadwalCollector(self):
		 
		NomorTagihan  = self.env["cnw.numbering.wizard"].getnumbering('CNW-TF',self.docdate)    
		ar_tf_id = self.env["ar.tf"].create({
												"name"      : NomorTagihan,
												"coll_id"   : self.coll_id.id,
												"jalur_id"  : self.jalur_id.id ,
		...
		for invoice in listinvoice:
			invoice.tf_id = ar_tf_id
			invoice.docduedate = self.docdate
			invoice.tf_date = self.docdate
			invoice.tg_date = self.docdate
			invoice.tf_number = NomorTagihan 
			invoice.tf_type = self.statustf
			invoice.tf_remarks = self.statustf
			invoice.tf_collector =  self.coll_id.name
			doctotal += invoice.total 
		
		ar_tf_id.doctotal = doctotal
