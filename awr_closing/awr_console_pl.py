...
class AWR_CONSOLE_PL(models.TransientModel):
    _name           = "cnw.console.pl"
    _description    = "cnw.console.pl"
    company_id      = fields.Many2many('res.company', string="Company",required=True)
    dateto          = fields.Date ("Date To", default=fields.Date.today()) 
    company2_id      = fields.Many2many('res.company', string="Company",required=True)
    dateto2          = fields.Date ("Compare Date To", default=fields.Date.today()) 
    ...
    def view_intercomp(self): 
        ...
        filename    = 'COMPARE_PL_'+ self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls    = 'COMPARE_PL_'+ self.env.user.company_id.db_name +   self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls2    = 'COMPARE_PL_'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamejson2    = 'COMPARE_PL_'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.json'
        filenamepdf = 'COMPARE_PL_'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.pdf'
