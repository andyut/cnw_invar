...
class AWR_InterComppurchase(models.TransientModel):
    _name           = "cnw.intercomp.purchase"
    _description    = "cnw.intercomp.purchase"
    ...
    def view_intercomp(self): 
        ...
        filename    = 'INTERCOMP_purchase'+ self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls    = 'INTERCOMP_purchase'+ self.env.user.company_id.db_name +   self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls2    = 'INTERCOMP_purchase'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamepdf = 'INTERCOMP_purchase'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.pdf'
            ...
            if comp.erp_type=='sap':
                msg_sql=  """
                                ...
                                sum(a.credit - a.debit)total from JDT1 A 
                                    ...
                                    AND (left(A.Account,3) ='119' or left(A.Account,1) ='5')   and a.transtype in (20,19,21,18,69) and left(convert(varchar,a.refdate,112),4) = left(@dateto ,4)
        ...
        if self.export_to =="xls":
            ...
            for line in listcom:
                ...
                worksheet.write (1,13 ,"Pembelian",header_format3)
        ...
        if self.export_to =="xls2":
            ...
            df.pivot_table(index=("cardname"),columns=("company"),aggfunc=np.sum,  values=["total"],fill_value="0",margins=True).to_csv(mpath + '/temp/'+ filenamexls2    ,sep="\t")
