...
class AWR_InterCompAR(models.TransientModel):
    ...
    def view_intercomp(self): 
        ...
        filename        = 'INTERCOMP_AR'+ self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls     = 'INTERCOMP_AR'+ self.env.user.company_id.db_name +   self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls2    = 'INTERCOMP_AR'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamepdf     = 'INTERCOMP_AR'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.pdf'
        ...
        for comp in self.company_id:
            ...
            if comp.erp_type =="sap":
                ...
                msg_sql="""
                                ...
                                select   '""" + comp.code_base + """' company  ,
                                c.company cardname, 
                                                sum(case when left(inv_date,6) <= left(@dateto ,4) + '01' and left(@dateto ,4)+'01' <= left(@dateto ,6) then total else 0 end )jan,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '02' and left(@dateto ,4)+'02' <= left(@dateto ,6)  then total else 0 end )feb,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '03' and left(@dateto ,4)+'03' <= left(@dateto ,6)  then total else 0 end )mar,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '04' and left(@dateto ,4)+'04' <= left(@dateto ,6)  then total else 0 end )apr,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '05' and left(@dateto ,4)+'05' <= left(@dateto ,6)  then total else 0 end )may,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '06' and left(@dateto ,4)+'06' <= left(@dateto ,6)  then total else 0 end )jun,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '07' and left(@dateto ,4)+'07' <= left(@dateto ,6)  then total else 0 end )jul,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '08' and left(@dateto ,4)+'08' <= left(@dateto ,6)  then total else 0 end )ags,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '09' and left(@dateto ,4)+'09' <= left(@dateto ,6)  then total else 0 end )sep,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '10' and left(@dateto ,4)+'10' <= left(@dateto ,6)  then total else 0 end )okt,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '11' and left(@dateto ,4)+'11' <= left(@dateto ,6)  then total else 0 end )nov,
                                                sum(case when left(inv_date,6) = left(@dateto ,4) + '12' and left(@dateto ,4)+'12' <= left(@dateto ,6)  then total else 0 end )des,
                                sum(total) total
                                From trade.getAR2_Detail( @dateto ) a 
                                    inner join trade.t_m_customer b on a.inv_Customer = b.ctm_Code
                                    inner join @table C ON b.ctm_taxNo = c.npwp
                                group by c.company           
                """
        ...
        if self.export_to =="xls":
            ...
            for line in listcom:
                ...
                worksheet.write (1,13 ,"Piutang Dagang",header_format3)
