...
class AWR_InterCompSALES(models.TransientModel):
    ...
    def view_pl(self): 
        ...
        filename    = 'SALES'+ self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls    = 'SALES'+ self.env.user.company_id.db_name +   self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamexls2    = 'SALES'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.xlsx'
        filenamepdf = 'SALES'+  self.env.user.company_id.db_name +  self.dateto.strftime("%Y%m%d")  + '.pdf'
        ...
        for company in datacompany:
            ...
            msg_sql=  """select  '""" + company["company"] + """' Company , '""" + company["codename"] + """' CompCode,
                        ...
                        sum(a.debit - a.credit )amount from JDT1 A 
                            ...
                            and CONVERT(VARCHAR,A.REFDATE,112)>= '""" +  self.datefrom.strftime("%Y%m%d") + """'
                            AND LEFT(A.Account,1) ='4'
